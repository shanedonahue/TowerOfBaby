    ...
    private string BuildExpandedText(TerrainWorldProfileSnapshot snapshot, LocomotionTelemetrySnapshot locomotionSnapshot)
    {
        ...
        else
        {
            ...
            if (snapshot.TerrainStatsEnabled)
            {
                ...
                _detailBuilder.AppendLine(
                    $"Edit detail dirty {snapshot.LastDeformDirtyBoundsVolume:0.0}/{ComputeAverage(snapshot.TotalEditedDirtyBoundsVolume, snapshot.DeformOperationCount):0.0} avg  promotions {snapshot.LastDeformEditDetailPromotionCount}/{ComputeAverage(snapshot.EditDetailPromotionCount, snapshot.DeformOperationCount):0.0} avg  vis {snapshot.LastDeformVisibleBlockCount}/{snapshot.LastDeformVisibleFinestBlockCount}  commit {snapshot.LastDeformVisibleCommitCount}  seam {snapshot.LastDeformSeamRefreshCount}  converge {snapshot.LastDeformVisibleConvergenceMs:0.00} ms");
                _detailBuilder.AppendLine(
