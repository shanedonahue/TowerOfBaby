    ...
    private void WriteLog()
    {
        ...
        if (_samples.Count > 0)
        {
            ...
            if (latestSnapshot != null)
            {
                ...
                {
                    builder.AppendLine($"LodStartupCriticalBlockCount: {latestSnapshot.StartupCriticalChunkCount}");
                    builder.AppendLine($"LodStartupCriticalSatisfiedBlockCount: {latestSnapshot.StartupCriticalSatisfiedCount}");
                    builder.AppendLine($"LodFullDesiredBlockCount: {latestSnapshot.StartupFullDesiredChunkCount}");
                    builder.AppendLine($"LodStartupBoostActive: {(latestSnapshot.StartupBoostActive ? 1 : 0)}");
                    builder.AppendLine($"LodTimeToFirstVisibleTerrainMs: {latestSnapshot.TimeToFirstVisibleTerrainMs:0.00}");
                    builder.AppendLine($"LodTimeToStartupCompleteMs: {latestSnapshot.TimeToStartupCompleteMs:0.00}");
                    builder.AppendLine($"LodRestoredFromStartupSnapshotCount: {latestSnapshot.RestoredFromStartupSnapshotCount}");
                    builder.AppendLine($"LodRestoredFromPersistedBlockCount: {latestSnapshot.RestoredFromPersistedBlockCount}");
                    builder.AppendLine($"LodProcedurallyGeneratedBlockCount: {latestSnapshot.ProcedurallyGeneratedBlockCount}");
                    ...
                    builder.AppendLine($"LodReleaseRequeues: {latestSnapshot.ReleaseRequeueCount}");
                    builder.AppendLine($"LodReleaseHeadOfLineAvoided: {latestSnapshot.ReleaseHeadOfLineAvoidedCount}");
        ...
        foreach (SamplePoint sample in _samples)
        {
            ...
            builder.AppendLine(string.Join(",",
                ...
                snapshot.LastReleaseRequeueCount.ToString(CultureInfo.InvariantCulture),
                snapshot.LastReleaseHeadOfLineAvoidedCount.ToString(CultureInfo.InvariantCulture),
                snapshot.LastAverageReleaseDeferredAgeMs.ToString("0.00", CultureInfo.InvariantCulture),
                ...
                snapshot.StartupCriticalChunkCount.ToString(CultureInfo.InvariantCulture),
                snapshot.StartupCriticalSatisfiedCount.ToString(CultureInfo.InvariantCulture),
                snapshot.StartupFullDesiredChunkCount.ToString(CultureInfo.InvariantCulture),
                snapshot.StartupBoostActive ? "1" : "0",
                snapshot.TimeToFirstVisibleTerrainMs.ToString("0.00", CultureInfo.InvariantCulture),
                snapshot.TimeToStartupCompleteMs.ToString("0.00", CultureInfo.InvariantCulture),
                snapshot.RestoredFromStartupSnapshotCount.ToString(CultureInfo.InvariantCulture),
                snapshot.RestoredFromPersistedBlockCount.ToString(CultureInfo.InvariantCulture),
                snapshot.ProcedurallyGeneratedBlockCount.ToString(CultureInfo.InvariantCulture),
